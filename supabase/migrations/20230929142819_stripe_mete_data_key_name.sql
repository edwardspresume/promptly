...
BEGIN
  ...
  VALUES (new.email, new.raw_user_meta_data->>'full_name', null, jsonb_build_object('metadata[supabase_profile_id]', new.id));
 

  -- Get the id of the newly inserted customer.
  stripe_customer_id := (SELECT id FROM stripe.customers WHERE attrs->'metadata'->>'supabase_profile_id' = new.id::TEXT);
